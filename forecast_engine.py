...
def fetch_pm25_window(start_date: date, end_date: date) -> pd.DataFrame:
    params = {
        "latitude": LATITUDE,
        "longitude": LONGITUDE,
        "start_date": start_date.isoformat(),
...
def predict_pm25(reference_date: date | None = None, use_live: bool = True) -> ForecastResult:
    ...
    if packaged_actual is not None and requested_target < today:
        ...
        is_demo = False
        is_degraded = False
        actual_source = "Packaged validation dataset"
    elif requested_target < today:
        try:
            target_date = requested_target
            air_quality_daily = fetch_pm25_window(reference_date - timedelta(days=80), target_date)
            history, actual_override, target_pm25_hours = split_air_quality_forecast(air_quality_daily, target_date)
            weather_daily = weather_climatology_for_target(target_date)
            data_note = (
                "Historical validation mode: Open-Meteo historical PM2.5 supplies the observed target value "
                "and prior PM2.5 lags. The prediction is generated by the research ML model."
            )
            weather_note = (
                "Weather features use same-season historical weather climatology from the packaged Nanjing dataset, "
                "because live weather forecasts are not available for past target dates."
            )
            data_mode = "Open-Meteo historical validation"
            operational_prediction = None
            operational_model_name = primary_model_name
            operational_source = "Research ML model"
            is_demo = False
            is_degraded = False
            actual_source = "Open-Meteo historical PM2.5"
            ...
            data_note = (
                ...
                "The app could not retrieve historical PM2.5 for this date, so it used a same-season PM2.5 baseline. "
            )
            weather_note = "Weather features use same-season historical weather averages from the packaged Nanjing dataset."
            data_mode = "historical fallback without observed PM2.5"
            target_pm25_hours = None
    ...
    if actual_override is not None:
        actual_pm25 = actual_override
    else:
        actual_pm25 = packaged_actual_pm25(target_date)
        if actual_pm25 is not None and actual_source is None:
            actual_source = "Packaged validation dataset"
    ...
    return ForecastResult(
        ...
        actual_pm25_source=actual_source,
